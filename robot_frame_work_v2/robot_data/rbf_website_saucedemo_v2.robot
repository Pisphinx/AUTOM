...
Ajouter des élements dans le panier : 
    ...
    ${items2}    Create List    sauce-labs-bolt-t-shirt    sauce-labs-backpack    sauce-labs-bike-light    sauce-labs-fleece-jacket    sauce-labs-onesie    test.allthethings()-t-shirt-(red)
    FOR    ${idForAdd}    IN    @{items2}
        Wait Until Page Contains Element   //button[@id='add-to-cart-${idForAdd}']    timeout=20s
        Log    --- Element ajouté avec succès. 🙌 🙌 '${idForAdd}'---
        #SeleniumLibrary.Get WebElement    //button[@id='add-to-cart-${idForAdd}']
        Click Element    //button[@id='add-to-cart-${idForAdd}']
    END
